<template>
  <div>
    <el-menu :collapse="isCollapse" default-active="1" :unique-opened="true" class="el-menu-vertical-demo" background-color="#409EFF" text-color="#fff" active-text-color="#ffd04b">
      <el-menu-item v-for="item in noChildren" :key="item.title" :index="item.title" @click="clickMenu(item)">
        <i :class="'el-icon-'+item.icon" />
        <span slot="title">{{ item.title }}</span>
      </el-menu-item>

      <el-submenu v-for="(item,index) in hasChildren" :key="index" :index="item.title">
        <template slot="title">
          <i :class="'el-icon-'+item.icon" />
          <span slot="title">{{ item.title }}</span>
        </template>
